...
struct TabulationHash {
    ...
    */
   template<typename T>
   static T forceinline naive_hash(const T value, const T (&table)[sizeof(T)][0xFF]) {
      T out = 0;
      unroll_loops for (auto i = 0; i < sizeof(T); i++) {
   ...
   static void forceinline gen_table(T table[COLUMNS][0xFF], unsigned int seed = 1) {
      ...
      unroll_loops for (auto c = 0; c < COLUMNS; c++) {
         unroll_loops for (auto r = 0; r < 0xFF; r++) {
            table[c][r] = 0;
            unroll_loops for (auto i = 0; i < COLUMNS; i++) {
