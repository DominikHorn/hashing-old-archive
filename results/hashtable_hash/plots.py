...
compilers = ["g++"]  # ["clang++", "g++"]
_csv = pandas.read_csv(f"hashtable_hash-{compilers[0]}.csv")
dataset_names = sorted(set(_csv['dataset']))
hash_functions = list(
    OrderedDict.fromkeys(list(_csv.sort_values('lookup_nanoseconds_total')['hash'])))
_colors = list(gen_colors(len(hash_functions)))
colors = {method: _colors[i] for i, method in enumerate(hash_functions)}


def gen_plot(key):
    for compiler in compilers:
        csv = pandas.read_csv(f'hashtable_hash-{compiler}.csv')
        ...
        for fig, dataset_name in enumerate(dataset_names):
            ...
            for l, load_factor in enumerate(reversed(load_factors)):
                for k, bucket_size in enumerate(reversed(bucket_sizes)):
                    ...
                    for j, hash_name in enumerate([m for m in hash_functions if m in set(ds['hash'])]):
                        series = list(
                            ds[ds['hash'] == hash_name].sort_values('reducer')[f'{key}_nanoseconds_per_key'])
                        width = 0.95 / len(hash_functions)
