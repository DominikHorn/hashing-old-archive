...
models = list(OrderedDict.fromkeys(list(_csv[_csv['reducer'] != 'do_nothing'].sort_values('total_nanoseconds_per_key')['model'])))
...
for compiler in compilers:
    ...
    for fig, dataset_name in enumerate(dataset_names):
        ...
        for k, load_factor in enumerate(reversed(sorted(set(dataset['load_factor'])))):
            ...
            # order preserving deduplication
            for j, model_name in enumerate([m for m in models if m in set(ds['model'])]):
                series = list(ds[ds['model'] == model_name].sort_values('reducer')['total_nanoseconds_per_key'])
                width = 0.95 / len(models)
            ...
            plt.grid(linestyle='--', linewidth=0.5)
            plt.title(f"total nanoseconds per key on {dataset_name}, load_factor {load_factor}, compiler {compiler}")
            plt.xticks(np.arange(len(reducers)) + (1.0 / len(reducers)), reducers)
            plt.ylabel('total nanoseconds per key')
            plt.xlabel('model reduction method')

            plt.legend(bbox_to_anchor=(0.5, -0.1), loc="upper center", ncol=7)

        plt.savefig(f"graphs/total_nanoseconds_per_key_{dataset_name}_{compiler}.png", bbox_inches='tight', pad_inches=0.5)
        plt.savefig(f"graphs/total_nanoseconds_per_key_{dataset_name}_{compiler}.pdf", bbox_inches='tight', pad_inches=0.5)
