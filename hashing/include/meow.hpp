/**
* The following code is largely based on/copied from official MeowHash code.
 * It is however not the original and should not be considered as such!
 *
 * MeowHash License:
 * zlib License
 *
...
#define pxor_clear(A, B) \
   A =                   \
      _mm_setzero_si128(); // NOTE(casey): pxor_clear is a nonsense thing that is only here because compilers don't detect xor(a, a) is clearing a :(
...
#define MEOW_MIX(r1, r2, r3, r4, r5, ptr)                                                                              \
   MEOW_MIX_REG(r1, r2, r3, r4, r5, _mm_loadu_si128((__m128i*) ((ptr) + 15)), _mm_loadu_si128((__m128i*) ((ptr) + 0)), \
                _mm_loadu_si128((__m128i*) ((ptr) + 1)), _mm_loadu_si128((__m128i*) ((ptr) + 16)))
...
struct MeowHash {
  ...
  private:
   ...
   constexpr static const meow_u8 MeowMaskLen[32] = {255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255,
   ...
   constexpr static const meow_u8 MeowDefaultSeed[128] = {
      };
   ...
   static void MeowBegin(meow_state* State, void* Seed128) {
      ...
      MEOW_DUMP_STATE("Seed", State->xmm0, State->xmm1, State->xmm2, State->xmm3, State->xmm4, State->xmm5, State->xmm6,
                      State->xmm7, 0);
...
#undef MEOW_DUMP_STATE
   ...
   static void MeowExpandSeed(meow_umm InputLen, void* Input, meow_u8* SeedResult) {
      meow_state State;
      meow_u64 LengthTab = (meow_u64)
         InputLen; // NOTE(casey): We need to always injest 8-byte lengths exactly, even on 32-bit builds, to ensure identical results
      meow_umm InjestCount = (256 / InputLen) + 2;

      MeowBegin(&State, (void*) MeowDefaultSeed);
