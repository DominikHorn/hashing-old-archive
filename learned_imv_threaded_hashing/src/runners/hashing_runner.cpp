...
#define RUN_NUMS 10 
#define HASH_MORSE_SIZE 0 //100000
...
volatile static char hash_g_lock = 0, hash_g_lock_morse = 0;
volatile static uint64_t hash_total_num = 0, hash_global_curse = 0, hash_global_upper, hash_global_morse_size;
Hashtable<KeyType, PayloadType> * ht;

typedef void (*HashBuildFun)(ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_input, Relation<KeyType, PayloadType> * rel_r_partition);
volatile static struct Fun {
  HashBuildFun fun_ptr;
  char fun_name[16];
} hash_pfun[4];
volatile static int hash_pf_num = 0;

typedef uint64_t (*HashProbeFun)(Relation<KeyType, PayloadType> * rel_s_partition, ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_output);
volatile static struct Fun1 {
  HashProbeFun fun_ptr;
  char fun_name[16];
} hash_pfun1[4];

//TOOD: to put the morse-driven implementation here !!!!!
void hash_build_rel_r_partition(ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_input, Relation<KeyType, PayloadType> * rel_r_partition)
{
...
void hash_build_rel_r_partition_imv(ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_input, Relation<KeyType, PayloadType> * rel_r_partition)
{
    Hashtable<KeyType, PayloadType>* ht = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_input)->ht;  
    BucketBuffer<KeyType, PayloadType>** overflowbuf = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_input)->overflowbuf;
...
void hash_build_rel_r_partition_learned(ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_input, Relation<KeyType, PayloadType> * rel_r_partition)
{   
    Hashtable<KeyType, PayloadType>* ht = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_input)->ht;  
    BucketBuffer<KeyType, PayloadType>** overflowbuf = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_input)->overflowbuf;
...
void hash_build_rel_r_partition_learned_imv(ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_input, Relation<KeyType, PayloadType> * rel_r_partition)
{
    ...
    __m512d v_zero512_double = _mm512_set1_pd(0.), half_avx = _mm512_set1_pd(0.5), fanout_avx = _mm512_set1_pd((double)FANOUT), fanout_minus_one_avx = _mm512_set1_pd((double)FANOUT - 1.), 
    ...
    for (uint64_t cur = 0; 1;) 
    {
        ...
        switch (state[k].stage) 
        {
            case 1:
            {
                ...
                general_reg_1_double = _mm512_mask_add_pd(general_reg_1_double, state[k].m_have_tuple, general_reg_1_double, half_avx);
...
uint64_t hash_probe_rel_s_partition(Relation<KeyType, PayloadType> * rel_s_partition, ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_output)
{
...
uint64_t hash_probe_rel_s_partition_imv(Relation<KeyType, PayloadType> * rel_s_partition, ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_output)
{
    Hashtable<KeyType, PayloadType>* ht = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_output)->ht;  
    uint64_t matches = 0;
...
uint64_t hash_probe_rel_s_partition_learned(Relation<KeyType, PayloadType> * rel_s_partition, ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_output)
{
    Hashtable<KeyType, PayloadType>* ht = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_output)->ht;  
...
uint64_t hash_probe_rel_s_partition_learned_imv(Relation<KeyType, PayloadType> * rel_s_partition, ETHNonPartitionJoinBuild<KeyType, PayloadType> *build_output)
{
    Hashtable<KeyType, PayloadType>* ht = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_output)->ht;  
    learned_sort_for_sort_merge::RMI<KeyType, PayloadType> * rmi = ((ETHNonPartitionJoinBuild<KeyType, PayloadType> *)build_output)->rmi;
    uint64_t matches = 0;
...
void * hash_thread(void * param)
{
    ...
    if (args->tid == 0) {
        ...
        strcpy(hash_pfun[1].fun_name, "IMV");
        strcpy(hash_pfun[0].fun_name, "Naive");

        hash_pfun[1].fun_ptr = hash_build_rel_r_partition_imv;
        hash_pfun[0].fun_ptr = hash_build_rel_r_partition;

        strcpy(hash_pfun1[1].fun_name, "IMV");
        strcpy(hash_pfun1[0].fun_name, "Naive");

        hash_pfun1[1].fun_ptr = hash_probe_rel_s_partition_imv;
        hash_pfun1[0].fun_ptr = hash_probe_rel_s_partition;

        hash_pf_num = 2;
#else
        strcpy(hash_pfun[0].fun_name, "Learned");
        strcpy(hash_pfun[1].fun_name, "Learned IMV");

        hash_pfun[0].fun_ptr = hash_build_rel_r_partition_learned;
        hash_pfun[1].fun_ptr = hash_build_rel_r_partition_learned_imv;

        strcpy(hash_pfun1[0].fun_name, "Learned");
        strcpy(hash_pfun1[1].fun_name, "Learned IMV");
        
        hash_pfun1[0].fun_ptr = hash_probe_rel_s_partition_learned;
        hash_pfun1[1].fun_ptr = hash_probe_rel_s_partition_learned_imv;
    ...
    for (int fid = 0; fid < hash_pf_num; ++fid) 
    {
        for (int rp = 0; rp < RUN_NUMS; ++rp) 
        {
            ...
            if(args->tid == 0){
                ...
                printf("---- %5s Build costs time (ms) = %10.4lf\n", hash_pfun[fid].fun_name, deltaT * 1.0 / 1000);
                hash_total_num = 0;
                hash_global_curse = 0;
            }
        
            if(!((fid == (hash_pf_num - 1)) && (rp == (RUN_NUMS - 1)))){
                if(args->tid == 0)
    ...
    for (int fid = 0; fid < hash_pf_num; ++fid) 
    {
        for (int rp = 0; rp < RUN_NUMS; ++rp) 
        {
            ...
            if(args->tid == 0){
                ...
                printf("---- %5s Probe costs time (ms) = %10.4lf\n", hash_pfun1[fid].fun_name, deltaT * 1.0 / 1000);
            }
        }
    }
...
void initialize_hash_thread_args(Relation<KeyType, PayloadType> * rel_r, 
...
int main(int argc, char **argv) 
{
    ...
    /*Tuple<KeyType, PayloadType>* s_tmp_training_sample_in;
    Tuple<KeyType, PayloadType>* s_sorted_training_sample_in;*/
    unsigned int SAMPLE_SZ_R = 0, SAMPLE_SZ_S = 0;
    ...
    /*initialize_hash_thread_args(&rel_r, &rel_s, ht, &rmi, rmi_params,
    ...
    initialize_hash_thread_args(&rel_r, &rel_r, ht, &rmi, rmi_params,
                                 SAMPLE_SZ_R, 0,
                                 tmp_training_sample_in, sorted_training_sample_in, r_tmp_training_sample_in,
                                 r_sorted_training_sample_in, r_tmp_training_sample_in, r_sorted_training_sample_in,
                                 &training_data, sample_count, sample_count_R, sample_count_R,
                                 slopes, intercepts,
                                 &barrier, joinresult, args_ptr);

    hash_global_curse = 0;
    hash_global_upper = rel_r.num_tuples;
    if(NUM_THREADS==1){
        hash_global_morse_size= rel_r.num_tuples;
    }else{
        hash_global_morse_size = HASH_MORSE_SIZE;
    }
    ...
    for(i = 0; i < NUM_THREADS; i++)
    {
        ...
        rv = pthread_create(&tid[i], &attr, hash_thread, (void*)&args[i]);
        if (rv){
